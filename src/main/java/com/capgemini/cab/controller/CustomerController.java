...
@Autowired    //wiring one object into another object
private CustomerServiceImpl customerServiceImpl;
	       ...
	       */
	          ...
	          //   http://localhost:9092/api/customer/login
			   //  {"username":"Bhavita","password":"reddy@123"}
              //   {"username":"jayasree","password":"sree@123"}
//
		@RequestMapping(method=RequestMethod.POST,value="/login")	// maps HTTP request with /login path to customerLogin method
		public ResponseEntity<String> CustomerLogin(@RequestBody Customer customer) // By using @RequestBody, all driver data will be passed to the server through JSON body
		{
			try
			{
				...
				logger.info("Customer login Details Found "+result);
				return new ResponseEntity<>(result,HttpStatus.OK);
			}
			catch (Exception e) 
			{
				logger.error("Customer Login Details Not Found: ");
		...
		//{"customerid":"563","firstname":"ganesh","lastname":"vinayak","gender":"M","contact":"9099531973","email":"ganesh@gmail.com","username":"ganesh","password":"ganesh@123","driver_id":512}
	...
	public ResponseEntity<String> addCandidate(@RequestBody  Customer customer)
	{
		...
		logger.info("Customer details are added "+result);
		return new ResponseEntity<>(result,HttpStatus.OK);
	}
	catch (Exception e) 
	{
		logger.error("Customer details are not found: ");
	 ...
	 */
	@ExceptionHandler(CustomerException.class)// handling exceptions in specific handler classes and/or handler methods  i.e in CustomerException class
	 public Map<String, String> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) // Exception is handled and passes the MethodArgumentNotValidException
	  {
